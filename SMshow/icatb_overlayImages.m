function [im,maxICAIM,minICAIM,minInterval,maxInterval] = icatb_overlayImages(icasig,structuralImage,icaDIM,structDIM,imageValues,maxvalue)
...
for i=1:numToOverlay
    if exist('maxvalue','var')
        [tempMax] = maxvalue; %max(icasig(i,:))
        [tempMin] = -maxvalue; %min(icasig(i,:));
    else 
